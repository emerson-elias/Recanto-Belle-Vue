export const suites = [

    {
        // INFORMAÇÕES PARA ROONS.JSX
        id: 'royale',
        title: 'Suíte Royale',
        subTitle: 'Vista frontal para o mar com varanda privativa',
        gallery: [
            '/assets/img/royale-1.jpg',
            '/assets/img/royale-2.jpg',
            '/assets/img/royale-3.jpg',
            '/assets/img/royale-4.jpg',
            '/assets/img/royale-5.jpg',
            '/assets/img/royale-6.jpg',
            '/assets/img/royale-7.jpg',
            '/assets/img/royale-8.jpg',
            '/assets/img/royale-9.jpg',
            '/assets/img/royale-10.jpg',
        ],
        categoria: 'Royale',
        description: 'A Suíte Royale oferece uma experiência premium com vista panorâmica para o mar, varanda privativa e decoração sofisticada. Ideal para casais ou famílias que buscam conforto, exclusividade e uma estadia inesquecível à beira-mar.',
        price: 'R$ 700 / Diária',
        icons: [
            { name: 'Wi-Fi', src: '/assets/svg/wifi.svg' },
            { name: 'Climatizado', src: '/assets/svg/cold.svg' },
            { name: 'TV (LCD)', src: '/assets/svg/tv.svg' },
            { name: 'Vista pro mar', src: '/assets/svg/balcony.svg' },
            { name: 'Acesso à praia', src: '/assets/svg/beach.svg' },
            { name: 'Refeições', src: '/assets/svg/buffet.svg' },
            { name: 'Lavanderia', src: '/assets/svg/washer.svg' },
            { name: 'Aquecedor', src: '/assets/svg/heater.svg' },
            { name: 'Proteção', src: '/assets/svg/grid.svg' },
            { name: 'Banho quente', src: '/assets/svg/bath.svg' }
        ],

        /* INFORMAÇÕES PARA DESCRIÇÃO*/
        meters: '25 metros²',
        people: '5 - 7 pessoas',
        pet: 'Pets Permitidos',
        inclusos: [
            'Cama king-size com enxoval em algodão egípcio de 600 fios, proporcionando extremo conforto e sofisticação.',
            'Banheira de hidromassagem com vista privilegiada para o mar, ideal para momentos relaxantes a dois.',
            'Varanda privativa equipada com rede, poltronas e iluminação suave para aproveitar o pôr do sol.',
            'Ar-condicionado split de última geração e ventilador de teto, garantindo um ambiente sempre agradável.',
            'Smart TV 50" com acesso liberado a Netflix, Prime Video e YouTube.',
            'Café da manhã gourmet servido diretamente na suíte, com menu personalizado.',
            'Wi-Fi de alta velocidade, perfeito para home office ou streaming.',
            'Frigobar abastecido com bebidas importadas e adega climatizada com seleção de vinhos.',
            'Roupões felpudos, chinelos e amenities veganos de luxo.',
        ],

        // INFORMAÇÕES PARA BANNER.JSX
        banner: '/assets/img/royale.jpg',
        letter: 'r',
        bannerSubtitle: 'Conforto e sofisticação',

        // INFORMAÇÕES PARA BIO.JSX
        bioTitle: 'Viva o luxo da Suíte Royale',
        bioCaption: 'Uma experiência premium e elegante à beira-mar',
        bioText: 'Com vista frontal para o oceano, varanda exclusiva e serviços de alto padrão, a Suíte Royale proporciona uma estadia memorável com todo o requinte que você merece.',
        bioImgOne: '/assets/img/royale-1.jpg',
        bioImgTwo: '/assets/img/royale-2.jpg',

        //INFORMAÇÕES PARA EXTRAS
        emphasis: [
            {
                title: 'Banheira panorâmica com vista privativa',
                text: 'A suíte conta com um amplo banheiro com banheira posicionada em frente a uma janela de vidro, oferecendo uma vista deslumbrante do mar para momentos únicos de relaxamento.'
            },
            {
                title: 'Varanda exclusiva frente mar',
                text: 'Relaxe ao som das ondas na sua varanda particular com vista direta para o oceano, perfeita para cafés da manhã ou noites estreladas.'
            },
            {
                title: 'Espaço ideal para famílias',
                text: 'Com capacidade para até cinco pessoas, a Suíte Royale garante conforto e privacidade em ambientes amplos e bem distribuídos.'
            },
            {
                title: 'Design sofisticado',
                text: 'Ambientes planejados com decoração moderna e elegante para uma estadia luxuosa em todos os detalhes.'
            }
        ]
    },

    {
        // INFORMAÇÕES PARA ROONS.JSX
        id: 'villa-real',
        title: 'Suíte Villa Real',
        subTitle: 'Vista para o jardim e varanda com vista lateral do mar',
        gallery: [
            '/assets/img/villa-1.jpg',
            '/assets/img/villa-2.jpg',
            '/assets/img/villa-3.jpg',
            '/assets/img/villa-4.jpg',
            '/assets/img/villa-5.jpg',
            '/assets/img/villa-6.jpg',
            '/assets/img/villa-7.jpg',
            '/assets/img/villa-8.jpg',
            '/assets/img/villa-9.jpg',
            '/assets/img/villa-10.jpg'
        ],
        categoria: 'Villa Real',
        description: 'Inspirada em villas mediterrâneas, a Suíte Villa Real combina charme clássico e conforto contemporâneo. Com amplo espaço interno, é perfeita para momentos de descanso com vista serena para o jardim tropical ou o pôr do sol à beira-mar.',
        price: 'R$ 400 / noite',
        icons: [
            { name: 'Wi-Fi', src: '/assets/svg/wifi.svg' },
            { name: 'Climatizado', src: '/assets/svg/cold.svg' },
            { name: 'TV (LCD)', src: '/assets/svg/tv.svg' },
            { name: 'Vista pro mar', src: '/assets/svg/balcony.svg' },
            { name: 'Acesso à praia', src: '/assets/svg/beach.svg' },
            { name: 'Refeições', src: '/assets/svg/buffet.svg' },
            { name: 'Lavanderia', src: '/assets/svg/washer.svg' },
            { name: 'Aquecedor', src: '/assets/svg/heater.svg' },
            { name: 'Proteção', src: '/assets/svg/grid.svg' },
            { name: 'Banho quente', src: '/assets/svg/bath.svg' }
        ],

        /* INFORMAÇÕES PARA DESCRIÇÃO*/
        meters: '22 metros²',
        people: '4 - 5 pessoas',
        pet: 'Pets Permitidos',
        inclusos: [
            'Suíte espaçosa com dois ambientes integrados, decoração clássica e detalhes em madeira nobre.',
            'Piscina privativa com borda infinita e vista para o jardim tropical.',
            'Terraço com espreguiçadeiras, pergolado e área de descanso ao ar livre.',
            'Sistema de som ambiente com conexão Bluetooth para criar a trilha sonora perfeita.',
            'Cozinha gourmet equipada com cooktop, micro-ondas, cafeteira e utensílios básicos.',
            'Serviço de quarto 24 horas com cardápio exclusivo.',
            'Estacionamento privativo em frente à suíte, com segurança monitorada.',
            'Café da manhã artesanal servido à beira da piscina ou na suíte.',
            'Ar-condicionado em todos os ambientes e TV por assinatura.',
        ],

        // INFORMAÇÕES PARA BANNER.JSX
        banner: '/assets/img/villa-real.jpg',
        letter: 'v',
        bannerSubtitle: 'Inspirada em villas',

        // INFORMAÇÕES PARA BIO.JSX
        bioTitle: 'Encante-se com a Villa Real',
        bioCaption: 'Charme mediterrâneo e conforto contemporâneo',
        bioText: 'Perfeita para momentos de descanso, a Suíte Villa Real oferece um ambiente espaçoso com vista para o jardim tropical e inspiração clássica para sua estadia.',
        bioImgOne: '/assets/img/villa-1.jpg',
        bioImgTwo: '/assets/img/villa-2.jpg',

        //INFORMAÇÕES PARA EXTRAS
        emphasis: [
            {
                title: 'Estilo mediterrâneo acolhedor',
                text: 'Inspirada nas vilas europeias, a suíte oferece charme clássico aliado ao conforto contemporâneo.'
            },
            {
                title: 'Vista para o jardim com pôr do sol',
                text: 'A varanda permite contemplar o entardecer em meio à natureza tropical da região.'
            },
            {
                title: 'Ambiente espaçoso e funcional',
                text: 'Ideal para famílias ou grupos menores que buscam um refúgio tranquilo e confortável.'
            },
            {
                title: 'Detalhes que encantam',
                text: 'Do mobiliário à iluminação, cada detalhe foi pensado para criar uma atmosfera acolhedora e charmosa.'
            }
        ]
    },

    {
        // INFORMAÇÕES PARA ROONS.JSX
        id: 'brisa-do-mar',
        title: 'Suíte Brisa do Mar',
        subTitle: 'Ventilação natural e vista lateral do mar',
        gallery: [
            '/assets/img/brisa.jpg',
            '/assets/img/brisa-1.jpg',
            '/assets/img/brisa-2.jpg',
            '/assets/img/brisa-3.jpg',
            '/assets/img/brisa-4.jpg',
            '/assets/img/brisa-5.jpg',
            '/assets/img/brisa-6.jpg',
            '/assets/img/brisa-7.jpg',
            '/assets/img/brisa-8.jpg',
            '/assets/img/brisa-9.jpg',
        ],
        categoria: 'Brisa do Mar',
        description: 'A Suíte Brisa do Mar é ideal para casais em busca de um refúgio romântico. Com ventilação natural e vista para o litoral, proporciona uma atmosfera leve, acolhedora e conectada com a natureza ao som das ondas do mar.',
        price: 'R$ 350 / noite',
        icons: [
            { name: 'Wi-Fi', src: '/assets/svg/wifi.svg' },
            { name: 'Climatizado', src: '/assets/svg/cold.svg' },
            { name: 'TV (LCD)', src: '/assets/svg/tv.svg' },
            { name: 'Vista pro mar', src: '/assets/svg/balcony.svg' },
            { name: 'Acesso à praia', src: '/assets/svg/beach.svg' },
            { name: 'Refeições', src: '/assets/svg/buffet.svg' },
            { name: 'Lavanderia', src: '/assets/svg/washer.svg' },
            { name: 'Aquecedor', src: '/assets/svg/heater.svg' },
            { name: 'Proteção', src: '/assets/svg/grid.svg' },
            { name: 'Banho quente', src: '/assets/svg/bath.svg' }
        ],

        /* INFORMAÇÕES PARA DESCRIÇÃO*/
        meters: '20 metros ²',
        people: '2 - 3 pessoas',
        pet: 'Pets Permitidos',
        inclusos: [
            'Aconchegante suíte com vista frontal para o mar e decoração em tons de azul e areia.',
            'Cama queen-size com colchão ortopédico e travesseiros antialérgicos.',
            'Varanda mobiliada com mesa para refeições ao ar livre e rede suspensa.',
            'Ar-condicionado silencioso e ventilador de teto com controle remoto.',
            'TV LED 40" com canais a cabo e conexão HDMI disponível.',
            'Banheiro com chuveiro pressurizado e aquecimento solar ecológico.',
            'Café da manhã regional servido no restaurante principal.',
            'Wi-Fi gratuito e de boa cobertura em toda a área da suíte.',
            'Serviço diário de limpeza com troca de toalhas e roupa de cama sob demanda.',
        ],

        // INFORMAÇÕES PARA BANNER.JSX
        banner: '/assets/img/brisa.jpg',
        bannerSubtitle: 'Conforto à beira-mar',
        letter: 'b',

        // INFORMAÇÕES PARA BIO.JSX
        bioTitle: 'Sinta a brisa do litoral',
        bioCaption: 'Um refúgio romântico com toque natural',
        bioText: 'Com ventilação cruzada e atmosfera leve, a Suíte Brisa do Mar é ideal para casais que buscam tranquilidade, conexão com a natureza e conforto à beira-mar.',
        bioImgOne: '/assets/img/brisa-1.jpg',
        bioImgTwo: '/assets/img/brisa-2.jpg',

        //INFORMAÇÕES PARA EXTRAS
        emphasis: [
            {
                title: 'Ventilação natural e brisa constante',
                text: 'A arquitetura da suíte favorece a circulação do ar e o frescor do litoral, sem abrir mão do conforto.'
            },
            {
                title: 'Ambiente romântico à beira-mar',
                text: 'O cenário ideal para casais que desejam aproveitar momentos tranquilos e especiais.'
            },
            {
                title: 'Conexão com a natureza',
                text: 'Desfrute da vista lateral do mar e da proximidade com a praia em um ambiente leve e sereno.'
            },
            {
                title: 'Decoração com elementos naturais',
                text: 'A madeira, os tecidos leves e os tons claros criam uma atmosfera suave e harmoniosa.'
            }
        ]
    },

    {
        // INFORMAÇÕES PARA ROONS.JSX
        id: 'terraço-dourado',
        title: 'Suíte Terraço Dourado',
        subTitle: 'Varanda ensolarada com vista parcial para o mar',
        gallery: [
            '/assets/img/terraço-1.jpg',
            '/assets/img/terraço-2.jpg',
            '/assets/img/terraço-3.jpg',
            '/assets/img/terraço-4.jpg',
            '/assets/img/terraço-5.jpg',
            '/assets/img/terraço-6.jpg',
            '/assets/img/terraço-7.jpg',
            '/assets/img/terraço-8.jpg',
            '/assets/img/terraço-9.jpg',
            '/assets/img/terraço-10.jpg',
        ],
        categoria: 'Terraço Dourado',
        description: 'A Suíte Terraço Dourado encanta com sua ampla varanda iluminada pelo sol da manhã. Um espaço perfeito para relaxar com uma vista privilegiada e ambiente elegante, ideal para quem busca momentos únicos e inesquecíveis.',
        price: 'R$ 300 / noite',
        icons: [
            { name: 'Wi-Fi', src: '/assets/svg/wifi.svg' },
            { name: 'Climatizado', src: '/assets/svg/cold.svg' },
            { name: 'TV (LCD)', src: '/assets/svg/tv.svg' },
            { name: 'Vista pro mar', src: '/assets/svg/balcony.svg' },
            { name: 'Acesso à praia', src: '/assets/svg/beach.svg' },
            { name: 'Refeições', src: '/assets/svg/buffet.svg' },
            { name: 'Lavanderia', src: '/assets/svg/washer.svg' },
            { name: 'Aquecedor', src: '/assets/svg/heater.svg' },
            { name: 'Proteção', src: '/assets/svg/grid.svg' },
            { name: 'Banho quente', src: '/assets/svg/bath.svg' }
        ],

        /* INFORMAÇÕES PARA DESCRIÇÃO*/
        meters: '23 metros ²',
        people: '2 - 4 pessoas',
        pet: 'Pets Permitidos',
        inclusos: [
            'Suíte ampla com design contemporâneo e detalhes dourados na decoração.',
            'Terraço privativo com jacuzzi aquecida, iluminação embutida e vista panorâmica do pôr do sol.',
            'Cama king-size com dossel, colchão de espuma de alta densidade e enxoval antialérgico.',
            'Ar-condicionado split e Smart TV com acesso a plataformas de streaming.',
            'Sala de estar integrada com mesa de jantar e poltronas de couro ecológico.',
            'Mini bar personalizado com drinks autorais, sucos naturais e snacks premium.',
            'Serviço de quarto personalizado com menu assinado por chef convidado.',
            'Estação de trabalho com iluminação natural e cadeira ergonômica, ideal para quem viaja a trabalho.',
        ],

        // INFORMAÇÕES PARA BANNER.JSX
        banner: '/assets/img/terraço-dourado.jpg',
        bannerSubtitle: 'Vista privilegiada',
        letter: 't',

        // INFORMAÇÕES PARA BIO.JSX
        bioTitle: 'Aprecie o sol da manhã',
        bioCaption: 'Vista ensolarada e momentos inesquecíveis',
        bioText: 'A Suíte Terraço Dourado encanta com sua ampla varanda e iluminação natural, sendo perfeita para relaxar com estilo e sofisticação.',
        bioImgOne: '/assets/img/terraço-1.jpg',
        bioImgTwo: '/assets/img/terraço-2.jpg',

        //INFORMAÇÕES PARA EXTRAS
        emphasis: [
            {
                title: 'Varanda ensolarada e espaçosa',
                text: 'A suíte oferece um terraço ideal para apreciar a luz natural e relaxar em qualquer momento do dia.'
            },
            {
                title: 'Vista parcial para o mar',
                text: 'Desfrute de uma vista encantadora com o brilho do sol refletido nas águas do oceano.'
            },
            {
                title: 'Toques de elegância em cada canto',
                text: 'Design moderno com atenção aos detalhes para uma estadia sofisticada e confortável.'
            },
            {
                title: 'Ideal para casais',
                text: 'Espaço pensado para duas pessoas que buscam tranquilidade e charme em um ambiente exclusivo.'
            }
        ]
    },

    {
        // INFORMAÇÕES PARA ROONS.JSX
        id: 'aurora',
        title: 'Suíte Aurora',
        subTitle: 'Ambiente minimalista com vista para o jardim',
        gallery: [
            '/assets/img/aurora-1.jpg',
            '/assets/img/aurora-2.jpg',
            '/assets/img/aurora-3.jpg',
            '/assets/img/aurora-4.jpg',
            '/assets/img/aurora-5.jpg',
            '/assets/img/aurora-6.jpg',
            '/assets/img/aurora-7.jpg',
            '/assets/img/aurora-8.jpg',
            '/assets/img/aurora-9.jpg',
            '/assets/img/aurora-10.jpg',
        ],
        categoria: 'Aurora',
        description: 'Inspirada nas cores suaves do nascer do sol, a Suíte Aurora proporciona uma estadia tranquila e relaxante. Com decoração clean e elegante, é o refúgio ideal para casais em busca de paz e conexão com a natureza.',
        price: 'R$ 470 / noite',
        icons: [
            { name: 'Wi-Fi', src: '/assets/svg/wifi.svg' },
            { name: 'Climatizado', src: '/assets/svg/cold.svg' },
            { name: 'TV (LCD)', src: '/assets/svg/tv.svg' },
            { name: 'Vista pro mar', src: '/assets/svg/balcony.svg' },
            { name: 'Acesso à praia', src: '/assets/svg/beach.svg' },
            { name: 'Refeições', src: '/assets/svg/buffet.svg' },
            { name: 'Lavanderia', src: '/assets/svg/washer.svg' },
            { name: 'Aquecedor', src: '/assets/svg/heater.svg' },
            { name: 'Proteção', src: '/assets/svg/grid.svg' },
            { name: 'Banho quente', src: '/assets/svg/bath.svg' }
        ],

        /* INFORMAÇÕES PARA DESCRIÇÃO*/
        meters: '18 metros ²',
        people: '3 - 4 pessoas',
        pet: 'Pets Permitidos',
        inclusos: [
            'Decoração suave com tons pastéis, plantas naturais e elementos inspirados na natureza.',
            'Cama queen-size com cabeceira em madeira de reflorestamento e iluminação indireta.',
            'Banheiro com ducha dupla, espelho antiembaçante e amenities veganos com aroma de lavanda.',
            'Varanda com vista para o jardim interno, perfeita para momentos de leitura e relaxamento.',
            'TV com acesso a streaming, conexão HDMI e sistema de som embutido.',
            'Cortinas blackout para garantir noites de sono tranquilas e sem interrupções.',
            'Café da manhã saudável com opções veganas e sem glúten, servido no restaurante ou na suíte.',
            'Ar-condicionado split e ventilador silencioso para garantir conforto térmico.',
        ],

        // INFORMAÇÕES PARA BANNER.JSX
        banner: '/assets/img/aurora.jpg',
        bannerSubtitle: 'Noites Estreladas',
        letter: 'a',

        // INFORMAÇÕES PARA BIO.JSX
        bioTitle: 'Desperte com leveza na Aurora',
        bioCaption: 'A beleza do nascer do sol em cada detalhe',
        bioText: 'Com decoração clean e atmosfera tranquila, a Suíte Aurora oferece um espaço acolhedor para quem busca paz e conforto em meio à natureza.',
        bioImgOne: '/assets/img/aurora-1.jpg',
        bioImgTwo: '/assets/img/aurora-2.jpg',

        //INFORMAÇÕES PARA EXTRAS
        emphasis: [
            {
                title: 'Decoração inspirada no nascer do sol',
                text: 'Tons suaves e iluminação natural proporcionam um ambiente de paz e renovação.'
            },
            {
                title: 'Ambiente minimalista e funcional',
                text: 'Perfeito para quem valoriza simplicidade, equilíbrio e conforto.'
            },
            {
                title: 'Vista relaxante para o jardim',
                text: 'Acorde com o som dos pássaros e aprecie o verde ao redor desde a sua varanda.'
            },
            {
                title: 'Ideal para relaxar e desconectar',
                text: 'A atmosfera tranquila convida ao descanso e ao bem-estar em meio à natureza.'
            }
        ]
    },

    {
        // INFORMAÇÕES PARA ROONS.JSX
        id: 'monte-sereno',
        title: 'Suíte Monte Sereno',
        subTitle: 'Ideal para casais | 21m² | Vista para o jardim com varanda tranquila',
        gallery: [
            '/assets/img/monte-1.jpg',
            '/assets/img/monte-2.jpg',
            '/assets/img/monte-3.jpg',
            '/assets/img/monte-4.jpg',
            '/assets/img/monte-5.jpg',
            '/assets/img/monte-6.jpg',
            '/assets/img/monte-7.jpg',
            '/assets/img/monte-8.jpg',
            '/assets/img/monte-9.jpg',
            '/assets/img/monte-10.jpg',
        ],
        categoria: 'Monte Sereno',
        description: 'Com vista para o jardim e decoração rústica-chique, a Suíte Monte Sereno é perfeita para quem deseja se desconectar da rotina. Oferece tranquilidade, contato com o verde e total conforto em um ambiente acolhedor.',
        price: 'R$ 520 / noite',
        icons: [
            { name: 'Wi-Fi', src: '/assets/svg/wifi.svg' },
            { name: 'Climatizado', src: '/assets/svg/cold.svg' },
            { name: 'TV (LCD)', src: '/assets/svg/tv.svg' },
            { name: 'Vista pro mar', src: '/assets/svg/balcony.svg' },
            { name: 'Acesso à praia', src: '/assets/svg/beach.svg' },
            { name: 'Refeições', src: '/assets/svg/buffet.svg' },
            { name: 'Lavanderia', src: '/assets/svg/washer.svg' },
            { name: 'Aquecedor', src: '/assets/svg/heater.svg' },
            { name: 'Proteção', src: '/assets/svg/grid.svg' },
            { name: 'Banho quente', src: '/assets/svg/bath.svg' }
        ],

        /* INFORMAÇÕES PARA DESCRIÇÃO*/
        meters: '21 metros ²',
        people: '3 - 5 pessoas',
        pet: 'Pets Permitidos',
        inclusos: [
            'Localizada na parte mais alta do resort, com vista panorâmica para a natureza e montanhas.',
            'Cama super king-size com enxoval térmico e almofadas extras para maior conforto.',
            'Lareira elétrica decorativa e iluminação ambiente ajustável.',
            'Banheiro espaçoso com banheira vitoriana, iluminação suave e piso aquecido.',
            'Varanda com deck de madeira, poltronas reclináveis e mesa de café.',
            'Smart TV 55", Wi-Fi ultra veloz e caixa de som inteligente.',
            'Menu de travesseiros disponível, com até 5 tipos diferentes à escolha.',
            'Café da manhã servido ao nascer do sol, com frutas da estação e pães artesanais.',
            'Serviço de camareira sob solicitação e aromaterapia opcional.',
        ],

        // INFORMAÇÕES PARA BANNER.JSX
        banner: '/assets/img/monte-sereno.jpg',
        bannerSubtitle: 'Vistas dos Montes',
        letter: 'm',

        // INFORMAÇÕES PARA BIO.JSX
        bioTitle: 'Encontre paz no Monte Sereno',
        bioCaption: 'Tranquilidade, liberdade e charme em meio ao verde',
        bioText: 'Com varanda voltada para o jardim e estilo rústico-chique, a Suíte Monte Sereno proporciona aconchego e desconexão com a rotina em um cenário encantador.',
        bioImgOne: '/assets/img/monte-1.jpg',
        bioImgTwo: '/assets/img/monte-2.jpg',

        //INFORMAÇÕES PARA EXTRAS
        emphasis: [
            {
                title: 'Estilo rústico-chique acolhedor',
                text: 'Combinando madeira, tecidos naturais e luz suave, a suíte oferece um verdadeiro refúgio no campo.'
            },
            {
                title: 'Varanda com vista para o jardim',
                text: 'Aprecie a tranquilidade e a beleza do verde em um espaço projetado para desacelerar.'
            },
            {
                title: 'Conexão com a natureza',
                text: 'Ideal para casais que desejam um momento de desconexão e reconexão com o essencial.'
            },
            {
                title: 'Ambiente silencioso e privativo',
                text: 'Desfrute da serenidade em um espaço que convida à contemplação e ao descanso pleno.'
            }
        ]
    }
]